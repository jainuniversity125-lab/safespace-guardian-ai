  ...
  async fetch(request: Request, env: unknown, ctx: unknown) {
    ...
    if (url.pathname === "/api/webhook") {
      ...
      try {
        ...
        // Create realtime alert for flagged content (DB trigger also does this,
        // but we add explicit insert for immediate response reliability)
        if (analysis.requires_review && ["medium", "high", "critical"].includes(analysis.severity)) {
          ...
          }).catch(() => {}); // Non-blocking - DB trigger is backup
        }
    ...
    if (url.pathname === "/api/webhook/supabase") {
      ...
      try {
        ...
        // Process Supabase realtime events
